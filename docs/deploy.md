...
- 設定は `apps/mcp-server/wrangler.jsonc`(name=`sf6-frame-data-mcp`, main=`src/index.ts`)。
- デプロイ後の URL: `https://sf6-frame-data-mcp.<your-subdomain>.workers.dev`

### 5. 動作確認

```sh
curl https://sf6-frame-data-mcp.<subdomain>.workers.dev/health   # => {"status":"ok"}
```
...
- カスタム MCP コネクタを追加し、URL に
  `https://sf6-frame-data-mcp.<subdomain>.workers.dev/mcp` を指定
